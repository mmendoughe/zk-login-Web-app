...
app.post('/run-script', (req, res) => {
  const { pass1, pass2, pass3, pass4, user1, user2, user3, user4, nonce1, nonce2, nonce3, nonce4, hash1, hash2, address1, address2, address3, address4} = req.body;
  exec(
    `./proof_generation/proof_generation.sh "${pass1}" "${pass2}" "${pass3}" "${pass4}" "${user1}" "${user2}" "${user3}" "${user4}" "${nonce1}" "${nonce2}" "${nonce3}" "${nonce4}" "${hash1}" "${hash2}" "${address1}" "${address2}" "${address3}" "${address4}"`,
    async (error, stdout, stderr) => {
      if (error) {
        console.error(`exec error: ${error}`);
        return res.status(500).send(stderr);
      }
      // Read generated proof file and send it back
      console.log(stdout);
